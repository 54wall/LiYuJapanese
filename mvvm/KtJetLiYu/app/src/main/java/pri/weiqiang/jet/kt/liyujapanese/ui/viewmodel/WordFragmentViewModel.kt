...
class WordFragmentViewModel(private val mSavedStateHandler:SavedStateHandle) : ViewModel() {
    ...
    init {
        wordList = Transformations.switchMap(mSavedStateHandler.getLiveData(QUERY_KEY,"新标日初级_01")){
            lessonId:String ->
            if (TextUtils.isEmpty(lessonId)){
                  return@switchMap VocabDatabase.getInstance(App.instance())?.wordDao()?.getWordByLessonId(lessonId)
            }
